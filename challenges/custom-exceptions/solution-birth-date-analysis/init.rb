...
def add_count_suffix(number)
  ...
  puts "Exception: Argument '#{number}' needs to be an Integer, is #{number.class}"
  exit
end
...
def analyze_date_of_birth(date_of_birth)
  ...
  years_old = years_old = (Date.today - date_of_birth).to_i / 365
  ...
  puts "Exception: Date '#{date_of_birth}' is not valid, analysis aborted."
end
